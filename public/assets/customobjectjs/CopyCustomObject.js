...
<div class="d-flex flex-wrap justify-content-between align-items-baseline mainCustomPropertyContainer currentCP-${length}">
<div class="row gx-2 gy-2>
    <div class="col-md-2 ">
        <input type="text" onChange="onChangeCustomInputValues(this)" name="propertylabel-${length}" id="propertylabel-${length}" property placeholder="Type Property Label" class="form-control" required>
    ...
    <select property name="propertytype-${length}" select-data-id="${length}" id="propertytype-${length}" onChange="selectEnumeration(this)" class="form-select customSelectOptionForModal" aria-label="Default select example">
    ...
    <div class="col-md-2 form-check form-switch">
     ...
     <div class="col-md-2form-check form-switch">
    ...
    <div>
         <button class="btn btn-danger"  removeRow="${length}" onClick="deleteCustomProperty(this)"><i class="fas fa-times"></i></button>
    </div>
    </div>
</div>`;

function addCustomProperty() {
  var lengthOfProperty = $("#customObjectFormProperty").children(".mainCustomPropertyContainer").length;
...
var enumerationSelectData = (length) => `
    <div class="d-flex flex-wrap justify-content-around align-items-baseline mainEnumerationOptionsContainer currentCEP-${length}">
        <div class="row gx-2 gy-2>
            <div class="col-md-2">
                <input childPropertyOfSelect onChange="onChangeCustomInputValues(this)" name="propertychildlabel-${length}" id="propertychildlabel-${length}" type="text" placeholder="Type Property Label" class="form-control" required>
                <div class="invalid-feedback">
                    Please provide a Property Name.
                </div>
            </div>
            <div class="col-md-2">
                <input childPropertyOfSelect onChange="onChangeCustomInputValues(this)" name="propertychildvalue-${length}" id="propertychildvalue-${length}" type="text" placeholder="Type Property Value" class="form-control" required>
                <div class="invalid-feedback">
                    Please provide a Property Name.
                </div>
            </div>
...
function addEnumerationOptions() {
  ...
  var lengthOfProperty = $("#EnumerationOptionsContainer").children(".mainEnumerationOptionsContainer").length;
  $("#EnumerationOptionsContainer").append(enumerationSelectData(lengthOfProperty));
}
...
function openCustomSelectModal(id) {
  ...
  if (
    ...
    Object.keys(JSON.parse(localStorage.getItem("customObjectUpdateDetailData"))).length > 0
  ) {
    var data = JSON.parse(localStorage.getItem("customObjectUpdateDetailData"));
...
function convertLocalDataToHubspotRequestData() {
  var submitDataFormat = { labels: {}, secondaryDisplayProperties: [], searchableProperties: [], requiredProperties: [], properties: [] };
  var data = {};
  if (localStorage.customObjectUpdateDetailData !== undefined) {
    data = JSON.parse(localStorage.getItem("customObjectUpdateDetailData"));
  }
  console.log("dsfds", data, localStorage.getItem("customObjectUpdateDetailData"));
  const checkForType = (type) => {
    if (type == "file" || type == "text" || type == "textarea") {
      ...
      return "number";
    } else if (type == "checkbox" || type == "booleancheckbox" || type == "radio" || type == "select") {
      return "enumeration";
    }
  };
  var options = ["select", "booleancheckbox", "checkbox", "radio", "select"];
  submitDataFormat["name"] = data.singularName.toLowerCase().trim();
  ...
  data.properties.map((ds) => {
    if (options.includes(ds.propertytype)) {
      ...
      if (ds.options.length) {
        ds.options.map((ol) => optionsData.push({ label: ol.propertychildlabel, value: ol.propertychildvalue }));
        submitDataFormat["properties"].push({
          name: ds.propertylabel,
          label: ds.propertylabel.replace(/(\w)(\w*)/g, function (g0, g1, g2) {
...
$("#submitCustomObjectFormData").on("click", function (event) {
  ...
  if (!submitDataFormat.primaryDisplayProperty || submitDataFormat.primaryDisplayProperty == "") {
  ...
  $(window).scrollTop(0);
  $.ajax({
    url: `/submit-copyformdata` + location.search,
    ...
    success: function (data, success, status) {
      // console.log(data);
      if (data.success) {
        showAlertMessage("success", "Custom Object Copied SuccessFully");
        $(".loading").hide();
        location.href = "/";
      } else {
        $(".loading").hide();
...
function onChangeCustomInputValues(event) {
  ...
  if (event.name === "customObjectName") {
    ...
    if (typeof propertyAttribute !== typeof undefined && propertyAttribute !== false) {
      ...
      if (event.name.includes("-")) {
        if (data["properties"][strings[1]] == undefined) {
          data["properties"].push({
            options: [],
            [strings[0]]: event.type === "checkbox" ? $(event).is(":checked") : event.value,
          });
        } else {
          data["properties"][strings[1]][strings[0]] = event.type === "checkbox" ? $(event).is(":checked") : event.value;
        }

        if ($(event).hasClass("primaryDisplayProperty")) {
          $(".primaryDisplayProperty").each(function (i, obj) {
            if (strings[1] == i) {
              console.log("aaaaa", { i, ssd: typeof i, strings }, data["properties"], [strings[1]], [strings[0]]);
              ...
              console.log("bbbbb", { i, strings }, data["properties"], [strings[1]], [strings[0]]);
          ...
          $(".secondaryDisplayProperty").each(function (i, obj) {
            if (strings[1] == i) {
              ...
              if ($(event).is(":checked")) {
                ...
                data["properties"][i]["propertyisprimaryDisplayProperty"] = false;
              }
            } else {
              // $(`#propertyisprimaryDisplayProperty-${i}`).prop("checked", false);
              // $(`#propertyisSecondayDisplay-${i}`).prop("disabled", false);
    ...
    } else if (typeof childPropertyAttribute !== typeof undefined && childPropertyAttribute !== false) {
      var strings = event.name.split("-");
      var parentSelectIndex = $("#modalSelectOptionId").val();
      console.log("parerrrr", parentSelectIndex, strings);
      if (event.name.includes("-")) {
        if ((data["properties"][parentSelectIndex] !== undefined && parentSelectIndex !== undefined) || parentSelectIndex !== "") {
          if (data["properties"][parentSelectIndex]["options"][strings[1]] == undefined) {
            data["properties"][parentSelectIndex]["options"].push({
              [strings[0]]: event.value,
            });
          } else {
            data["properties"][parentSelectIndex]["options"][strings[1]][strings[0]] = event.value;
          }
        }
...
if (Object.keys(JSON.parse(updateObjectIdDetails)).length > 0) {
  ...
  updateCustomDataForm["singularName"] = updateObjectIdDetailsData.labels.singular;
  ...
  updateObjectIdDetailsData.properties.map((pr, i) => {
    updateCustomDataForm["properties"].push({
      ...
      ["propertyisRequired"]: updateObjectIdDetailsData.requiredProperties.includes(pr.name),
      ["propertyisprimaryDisplayProperty"]: updateObjectIdDetailsData.primaryDisplayProperty.includes(pr.name),
      ["propertyisSecondayDisplay"]: updateObjectIdDetailsData.secondaryDisplayProperties.includes(pr.name),
      ["propertyisSearchable"]: updateObjectIdDetailsData.searchableProperties.includes(pr.name),
      options: [],
    });
  ...
  localStorage.setItem("customObjectUpdateDetailData", JSON.stringify(updateCustomDataForm));
  ViewJsonFormatData();
}
...
function ViewJsonFormatData() {
  ...
  var htmldata = `<pre class="prettyprint convertToJson"><code>${JSON.stringify(data, undefined, 5)}</code><pre>`;
